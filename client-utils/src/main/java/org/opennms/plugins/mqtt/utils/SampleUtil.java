...
public class SampleUtil {
    ...
    private static List<Certificate> loadCertificatesFromFile(final String filename) {
        ...
        if (!file.exists()) {
            System.out.println("Certificate file is not found :"+file.getAbsolutePath());
            return null;
        }
        ...
        try (BufferedInputStream stream = new BufferedInputStream(new FileInputStream(file))) {
            ...
            System.out.println("Failed to load certificate file " + filename+ "exception "+e);
        }
        return null;
    }
    ...
    private static PrivateKey loadPrivateKeyFromFile(final String filename, final String algorithm) {
        ...
        File file = new File(filename);
        if (!file.exists()) {
            System.out.println("Private key file not found: " + file.getAbsolutePath());
            return null;
        }
        try (DataInputStream stream = new DataInputStream(new FileInputStream(file))) {
            privateKey = PrivateKeyReader.getPrivateKey(stream, algorithm);
        } catch (IOException | GeneralSecurityException e) {
            System.out.println("Failed to load private key from file " + filename+ "  exception "+e);
        }
